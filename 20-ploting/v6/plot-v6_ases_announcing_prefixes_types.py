...
arquivo = '19-preparing_data_for_plot/v6/ases/ases_types.txt'

# Ler o arquivo em um DataFrame
df = pd.read_csv(arquivo, sep='|', comment='#', header=0, names=["date", "internet","business","education","government","hosting","isp","none"])
...
#fig.suptitle('ASes anunciando prefixos IPv6 de acordo com tipo', fontsize=FONTSIZE_TITLE, fontweight='bold',  y=0.95, x=0.53)
#ax1.set_title(SUBTITLE, fontsize=FONTSIZE_SUBTITLE, pad=15)

# Adicionar as linhas das regiões
for tipo in tipos:
    ax1.plot(df['date'], df[tipo], linestyle='-', label=tipo, color=CORES_TYPES[tipo], linewidth=2)

# Adicionar a linha para a Internet
ax1.plot(df['date'], df['internet'], linestyle='-', label='internet', color=CORES_TYPES['internet'], linewidth=3)
...
output_folder = '20-ploting/v6/plot/ases'
output_path = os.path.join(output_folder, f'v6_ases_announcing_prefixes_types.png')
