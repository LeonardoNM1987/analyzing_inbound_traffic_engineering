...
arquivo = '19-preparing_data_for_plot/v6/moreSpecific/more_specific_deploy_regions.txt'

# Ler o arquivo em um DataFrame
df = pd.read_csv(arquivo, sep='|', comment='#', header=0)

# Renomear as colunas para facilitar a manipulação
df.columns = ["regiao", "data", "observed_ases", "using_moreSpecific","moreSpecific_fraction"]
...
dfs = {regiao: df[df['regiao'] == regiao][['data', 'observed_ases', 'using_moreSpecific']].reset_index(drop=True) for regiao in regioes}
...
for regiao in regioes:
    ...
    dfs[regiao]['Porcentagem'] = (dfs[regiao]['using_moreSpecific'] / dfs[regiao]['data'].map(total_internet)) * 100
    
      
    
    
    # # Calcular a porcentagem de ASNs com prepends observados
    # dfs[regiao]['Porcentagem'] = (dfs[regiao]['using_moreSpecific'] / dfs[regiao]['observed_ases']) * 100
...
# fig.suptitle('Fração de ASes usando Prefixo mais Específico ao longo do tempo', fontsize=FONTSIZE_TITLE, fontweight='bold', y=0.95)
...
for regiao in regioes:
    ...
    ax1.plot(df_regiao['data'], df_regiao['Porcentagem'], label=label, color=CORES[regiao], linestyle=linestyle, linewidth=linewidth)
...
output_folder = '20-ploting/v6/plot/moreSpecific'  # Substitua pelo caminho da pasta de saída desejada
current_time = datetime.datetime.now().strftime('%d%m%y.%H%M')
output_path = os.path.join(output_folder, f'v6_moreSpecific_deploy_fraction_regions.png')
