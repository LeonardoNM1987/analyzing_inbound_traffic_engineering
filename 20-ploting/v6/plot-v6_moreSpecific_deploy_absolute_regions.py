...
df.columns = ["regiao", "data", "total_asns", "using_moreSpecific", 'fractional_values']
...
dfs = {regiao: df[df['regiao'] == regiao][['data', 'using_moreSpecific']].reset_index(drop=True) for regiao in regioes}
...
#fig.suptitle('Quantidade de ASes usando Prefixo Mais Específico ao longo do tempo', fontsize=FONTSIZE_TITLE, fontweight='bold', y=0.95, x=0.53)
...
for regiao in regioes:
    ...
    ax1.plot(df_regiao['data'], df_regiao['using_moreSpecific'], label=label, color=CORES[regiao], linestyle=linestyle, linewidth=linewidth)
...
output_path = os.path.join(output_folder, f'v6_moreSpecific_deploy_absolute_regions.png')
