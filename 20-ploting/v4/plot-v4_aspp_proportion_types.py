...
import numpy as np
import pandas as pd
from _plot_compact_config import (SUBTITLE,FIGSIZE,FONTSIZE_LABEL,FONTSIZE_TITLE,FONTSIZE_SUBTITLE,FONTSIZE_LEGEND,FONTSIZE_TICKS,DATE_FORMAT,GRID_MAJOR,GRID_MINOR, CORES_TYPES)
...
arquivo = '19-preparing_data_for_plot/v4/aspp_proportion_types.txt'
df = pd.read_csv(arquivo, sep='|')

# Filtrar dados para a região 'internet'
df_global = df[df['as_type'] == type]
...
categorias = ['0.1% a 24.9% dos anúncios', '25% a 49.9% dos anúncios', '50% a 74.9% dos anúncios', '75% a 100% dos anúncios']

# Criar o gráfico
fig, ax = plt.subplots(figsize=FIGSIZE)
# Adicionar título e subtítulo
#fig.suptitle(f'Proporção de uso de ASPP de cada AS em seus anúncios \n por tipo de AS - {type} ', fontsize=FONTSIZE_TITLE, fontweight='bold',y=0.95, x=0.53)
...
# ax.legend(
#     loc='upper center',
#     title='Proporção de anúncios em que os ASes usam ASPP',
...
output_folder = '20-ploting/v4/plot_dissertacao/aspp'
output_path = os.path.join(output_folder, f'v4_aspp_proportion_type_{type}.png')
