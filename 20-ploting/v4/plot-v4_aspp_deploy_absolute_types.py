...
dfs = {tipo: df[df['tipo'] == tipo][['data', 'quantidade_prepends']].reset_index(drop=True) for tipo in tipos}
...
#fig.suptitle('Quantidade de ASes usando ASPP de acordo com o tipo de AS', fontsize=FONTSIZE_TITLE, fontweight='bold', y=0.95, x=0.53)
...
for tipo in tipos:
    ...
    ax1.plot(df_tipo['data'], df_tipo['quantidade_prepends'], label=label, color=CORES_TYPES[tipo], linestyle=linestyle, linewidth=linewidth)
...
output_path = os.path.join(output_folder, f'v4_aspp_deploy_absolute_types.png')
