...
arquivo = '19-preparing_data_for_plot/v4/aspp_proportion_regions.txt'
...
#fig.suptitle(f'Proporção de uso de ASPP de cada AS em seus anúncios - {regiao} ', fontsize=FONTSIZE_TITLE, fontweight='bold',y=0.95, x=0.53)
...
for (categoria, proporcao), cor in zip(proporcoes_global.items(), cores_global):
    ax.plot(df_global['date'], proporcao, label=categoria, color=cor, linewidth=2)
...
# #Marca dagua
# fig.text(
#     0.5, 0.5,  # Posição central no gráfico
#     'LACNIC',  # Texto da marca d'água
#     fontsize=30,  # Tamanho da fonte
#     color='gray',  # Cor da marca d'água
#     alpha=0.3,  # Transparência da marca d'água
...
output_path = os.path.join(output_folder, f'v4_aspp_proportion_region_{regiao}.png')
