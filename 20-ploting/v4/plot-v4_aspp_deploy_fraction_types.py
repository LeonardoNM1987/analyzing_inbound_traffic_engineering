...
dfs = {tipo: df[df['tipo'] == tipo][['data', 'total_asns', 'quantidade_prepends']].reset_index(drop=True) for tipo in tipos}
...
for tipo in tipos:
    ...
    dfs[tipo]['Porcentagem'] = (dfs[tipo]['quantidade_prepends'] / dfs[tipo]['data'].map(total_internet)) * 100
    
    
    
    # # Calcular a porcentagem de ASNs com prepends observados
    # dfs[tipo]['Porcentagem'] = (dfs[tipo]['quantidade_prepends'] / dfs[tipo]['total_asns']) * 100
...
output_path = os.path.join(output_folder, f'v4_aspp_deploy_fraction_types.png')
