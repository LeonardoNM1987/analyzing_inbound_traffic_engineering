...
def parse_line(line):
    ...
    if len(parts) < 6 or parts[3] == 'None':
        return None
    try:
        ipaddress.IPv6Network(parts[0])  # Validate IPv6 prefix
    except ValueError:
        return None
    return {
        'prefix': parts[0],
...
def process_file(input_path, output_path):
    ...
    print(f"Linhas processadas no arquivo {input_path}: {processed_lines}")
...
def process_all_files(input_dir, output_dir):
    ...
    input_files = glob.glob(os.path.join(input_dir, 'v6_prefixes_policies_*.txt'))
    ...
    for input_file in input_files:
        ...
        output_file = os.path.join(output_dir, f'v6_moreSpecific_ases_details_{date_part}.txt')
        process_file(input_file, output_file)

# Paths
input_dir = f'05-prefixes_policies_regions_and_types_saned/v6_prefixes/'
output_dir = f'08-more_specific_results/v6/ases_details/'
