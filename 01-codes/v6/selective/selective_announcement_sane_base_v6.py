...
input_folder = '09-selective_announcement_database/v6/'
delegatedPath = '03-delegated/'
typesFile = '02-resources/as_types.csv'
output_folder = '10-selective_announcement_regions_and_types_saned/v6/'
summary_file = os.path.join(output_folder, 'summary_report.txt')

# Certificar que a pasta de saída existe
os.makedirs(output_folder, exist_ok=True)

# Função para validar se é IPv6 e obter o comprimento do prefixo
def is_ipv6(prefix):
    try:
        ip = ipaddress.IPv6Network(prefix, strict=False)
        return True, ip.prefixlen  # Retorna True e o comprimento do prefixo
    except ValueError:
        return False, None  # Retorna False e None caso não seja um IPv6 válido
...
def processar_prefixos(prefix_data):
    ...
    for item in prefixos:
        ...
        # Verificar se o prefixo é IPv6
        is_valid_ipv6, prefix_length = is_ipv6(prefix)
        if not is_valid_ipv6 or prefix_length < 16 or prefix_length > 48:
...
def processar_por_intervalo(input_folder, output_folder, start_year, end_year, start_month, end_month):
    ...
    with open(summary_file, 'w') as resumo:
        ...
        while data_atual <= data_final:
            ...
            input_file = os.path.join(input_folder, f'v6_prefix_origin_{date_str}.txt')
            output_file = os.path.join(output_folder, f'v6_prefix_origin_complete_{date_str}.txt')
