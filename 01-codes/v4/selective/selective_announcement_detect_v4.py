...
def disaggregate_to_24(prefix):
    """Disaggregate a given prefix into /24s."""
...
def analyze_selective_announcement(input_line):
    ...
    for prefix, neighbors in prefix_neighbors.items():
        ...
        # Disaggregate prefix into /24 and track their visibility
        disaggregated_prefixes = disaggregate_to_24(prefix)
        for subprefix in disaggregated_prefixes:
            if subprefix not in visibility_24:
                visibility_24[subprefix] = set()
            visibility_24[subprefix].update(neighbors)

    use_selective = selective_prefixes > 0

    # Metrics for AS-level proportionality and intensity
    total_24s = len(visibility_24)
    selective_24s = sum(1 for neighbors in visibility_24.values() if len(neighbors) < len(all_neighbors))
    as_proportionality = selective_24s / total_24s if total_24s > 0 else 0

    # Metrics for per-/24 intensity
    intensities = [len(neighbors) / len(all_neighbors) for neighbors in visibility_24.values()]
...
def process_input_files(input_dir, output_dir, start_year, end_year, start_month=1, end_month=12):
    ...
    for year in range(start_year, end_year + 1):
        for month in range(start_month, end_month + 1):
            ...
            input_file = f"v4_prefix_origin_complete_{date}.txt"
...
if __name__ == "__main__":
    ...
    input_directory = '10-selective_announcement_regions_and_types_saned/v4'  # Diretório de entrada
    output_directory = '11-selective_announcement_results/v4'  # Diretório de saída
