...
def main():
    input_directory = '05-prefixes_policies_regions_and_types_saned/v4_prefixes/'
    output_directory = '07-aspp_results/v4/proportion/'
    
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    
    for filename in os.listdir(input_directory):
        if filename.startswith('v4_prefixes_policies_complete_') and filename.endswith('.txt'):
            input_file = os.path.join(input_directory, filename)
            date = filename.split('_')[-1].split('.')[0]
            output_file = os.path.join(output_directory, f'v4_aspp_proportion_{date}.txt')
