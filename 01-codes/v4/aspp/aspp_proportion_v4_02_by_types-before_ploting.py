...
dir_path = f'07-aspp_results/v4/proportion/'
output_file = f'19-preparing_data_for_plot/v4/aspp_proportion_types.txt'
...
results = pd.DataFrame(columns=['as_type', 'data', 'from1to24', 'from25to49', 'from50to74', 'from75to100'])
...
if not os.path.exists(dir_path):
    ...
    files = glob.glob(os.path.join(dir_path, 'v4_aspp_proportion_*.txt'))
    ...
    if not files:
        ...
        for file_path in files:
            ...
            date_str = file_name.replace('v4_aspp_proportion_', '').replace('.txt', '')
            ...
            df = df[df['as_type'].isin(["business", "education", "government", "hosting", "isp", "none"])]
            ...
            new_row = pd.DataFrame([{'as_type': 'internet', 'data': date_str, **internet_category_counts}])
            results = pd.concat([results, new_row], ignore_index=True)

            # Adicionar resultados tipificados
            for type in df['as_type'].unique():
                typing_df = df[df['as_type'] == type]
                typing_category_counts = { 'from1to24': 0, 'from25to49': 0, 'from50to74': 0, 'from75to100': 0 }
                for percent in typing_df['prepend_percentage']:
                    category = get_category(percent)
                    if category:
                        typing_category_counts[category] += 1

                new_row = pd.DataFrame([{'as_type': type, 'data': date_str, **typing_category_counts}])
                results = pd.concat([results, new_row], ignore_index=True)

# Ordenar os resultados por data e região
results = results.sort_values(by=['as_type', 'data'])
...
with open(output_file, 'w') as f:
    ...
    for index, row in results.iterrows():
        f.write(f"{row['as_type']}|{row['data']}|{row['from1to24']}|{row['from25to49']}|{row['from50to74']}|{row['from75to100']}\n")
